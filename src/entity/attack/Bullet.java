...
public class Bullet extends Attack {
    ...
    private void setTarget(Engine eng, int startX, int startY){
        if(eng.player.direction.equalsIgnoreCase("up")){
            dx = 0;
            dy = -getSpeed();
        } else if(eng.player.direction.equalsIgnoreCase("down")){
            dx = 0;
            dy = getSpeed();
        } else if(eng.player.direction.equalsIgnoreCase("left")){
            dx = -getSpeed();
            dy = 0;
        } else if(eng.player.direction.equalsIgnoreCase("right")){
            dx = getSpeed();
            dy = 0;
        }
    }
